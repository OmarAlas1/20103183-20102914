...
void DrawHemisphere(float x , float y, float radiusX , float radiusY){
    ...
    for (int i = 0; i >= -180; i--){
       radian = i * M_PI / 180;
       glVertex2f(x + radiusX * cos(radian), y + radiusY * sin(radian));
  }
   glEnd();
}
...
int DrawGLScene(GLvoid)									// Here's Where We Do All The Drawing
{
    ...
    float x = fmod(translateX,8);
    // draw some fish
    drawFish(x , -3,0.7,0.5,0.4);
    drawFish(x - 6,-3,0.7,0.5,0.4);
    drawFish(x + 6 ,-3,0.7,0.5,0.4);
    drawFish(x + 6,-3,0.7,0.5,0.4);
    drawFish(x + 8,-2,0.7,0.5,0.4);
    // translate for fish
    translateX-= 0.03;
    translateY+= 0.03;
    //DrawEllipse(5 ,5 , 1,2);
	...
	int			nCmdShow)			// Window Show State
{
	...
	if (!CreateGLWindow("20103183 && 20102914", 1200 , 800, 16, fullscreen))
	{
